...
import app.blueprints.incomings.consults as Consults

@incomings.route("/", methods = ["GET","POST"])
@login_required
@role_required(['admin'])
# @limiter.limit('10/minute')
def incomings_app():    

    return render_template('incomings.html', title = 'Ingresos')

@incomings.route("/api/table", methods = ["GET","POST"])
@login_required
@role_required(['admin'])
# @limiter.limit('10/minute')
def incomings_api_table():
    ...
    return Consults.get_incomings(inicial, final)